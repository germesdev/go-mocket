...
func (mc *MockCatcher) FindResponse(query string, args []driver.NamedValue, connstring string) *FakeResponse {
	mc.mu.Lock()
	defer mc.mu.Unlock()
	if mc.Logging {
	...
	for _, resp := range mc.Mocks {
		if resp.IsMatch(query, args, connstring) {
...
func (mc *MockCatcher) NewMock(connection string) *FakeResponse {
	mc.mu.Lock()
	defer mc.mu.Unlock()
	fr := &FakeResponse{
		Exceptions: &Exceptions{},
...
// Returns true either when nothing to compare or deep equal check passed
func (fr *FakeResponse) isConnMatch(connstring string) bool {
	fr.mu.Lock()
	defer fr.mu.Unlock()
	return fr.Connection == connstring
}
...
func (fr *FakeResponse) IsMatch(query string, args []driver.NamedValue, connstring string) bool {
	fr.mu.Lock()
	...
	fr.mu.Unlock()
	return fr.isQueryMatch(query) && fr.isArgsMatch(args) && fr.isConnMatch(connstring)
}
